...
class GameSettingsView(BaseView):
        ...
        async def fetch_page(page: int) -> tuple[Embed, int]:
            async with game_service_ctx() as game_service:
                result = await game_service.get_players(
                    self.game_id, page=page, page_size=10, status=PlayerStatusEnum.ACCEPTED
                )
    ...
    async def player_characters_button(self, _: Button, inter: MessageInteraction) -> None:
        ...
        async def fetch_page(page: int) -> tuple[Embed, int]:
            async with character_service_ctx() as character_service:
                result = await character_service.get_by_game_id(self.game_id, page=page, page_size=5)
    ...
    @button(label="Close", style=ButtonStyle.danger, custom_id="game_settings:close", row=2)
