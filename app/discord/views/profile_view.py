...
class ProfileView(BaseView):
    ...
    @button(label="Reset Second Discord", style=ButtonStyle.danger, custom_id="profile:reset_discord", row=0)
    async def reset_discord_button(self, _: Button, inter: MessageInteraction) -> None:
        await inter.response.defer(ephemeral=True)
        async with user_service_ctx() as user_service:
            user = await user_service.get_user_by_discord(inter.author.id)
            await user_service.detach_secondary_discord_id(user.id)
        await inter.followup.send("✅ Второй Discord ID успешно сброшен", ephemeral=True)
